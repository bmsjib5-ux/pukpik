...
- แตะบนจอเพื่อสั่งให้มันเดินไปหาจุดนั้น หรือใช้ปุ่มลูกศรบังคับเอง
- มีเสียงคลิกและแรงสั่นตอนกดปุ่ม ปิดแยกกันได้ในเมนูตั้งค่า
...
ทุกปุ่ม (A B C, ไอคอนบนเครื่อง, ปุ่มในเมนูตั้งค่า, ปุ่มเลือกช่องบันทึก) มีทั้งเสียงคลิกและแรงสั่น
เสียงต่างระดับกันตามหน้าที่ของปุ่ม — `A` 700 Hz, `B` 880 Hz, `C` 420 Hz

- ปิด/เปิดแยกกันได้ที่ **ตั้งค่า → เสียง** และ **ตั้งค่า → สั่นเมื่อกดปุ่ม** (เปิดทั้งคู่เป็นค่าเริ่มต้น)
- เสียงสร้างจาก Web Audio API ไม่มีไฟล์เสียงแนบ และจะปลุก `AudioContext` ให้เองหลังการกดครั้งแรก
- แรงสั่นใช้ `navigator.vibrate()` — Android รองรับ ส่วน iOS Safari ยังไม่รองรับ (จะเงียบไปเฉยๆ ไม่ error)
- ผู้ที่เคยเล่นมาก่อนและมีค่าตั้งไว้แล้ว ระบบจะใช้ค่าเดิม ถ้าเสียงไม่ดังให้ไปเปิดที่เมนูตั้งค่า

## ความเร็วเวลา

มีให้เลือกสองระดับ: **ปกติ** (1 tick ต่อวินาที) และ **เร็ว x2**
ถ้าเคยตั้ง x4 ไว้จากเวอร์ชันก่อน ระบบจะดึงกลับมาที่ x2 ให้เองตอนเปิดครั้งถัดไป

## เดินเล่นบนหน้าจอ

- **แตะจุดไหนบนจอ** สัตว์เลี้ยงจะเดินไปหาจุดนั้น (เร็วกว่าตอนเดินเล่นเอง 2 เท่า) แล้วกลับไปเดินเรื่อยเปื่อยต่อ
- **ลากเมาส์** บนจอเพื่อนำทางไปเรื่อยๆ ได้ — บนมือถือใช้แตะอย่างเดียว เพื่อให้ยังเลื่อนหน้าจอได้ตามปกติ
- **ปุ่มลูกศร** บังคับทีละก้าว (ซ้าย/ขวา/ขึ้น/ลง) ส่วนในเมนูปุ่มลูกศรยังใช้เลื่อนรายการเหมือนเดิม
- เดินไปถึงที่หมายไกลๆ จะได้ความสุขเพิ่มเล็กน้อยและเสียพลังนิดหน่อย
- สั่งไม่ได้ตอนเป็นไข่ ตอนหลับ หรือตอนเปิดเมนูค้างอยู่
